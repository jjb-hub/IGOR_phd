...
class DataSelection (Cachable): 
    ...
    sex: str | list  = field(kw_only=True, default=None)
    ...
    def validate_inputs(self):
        ...
        def validate_attribute(column_name, attribute):
            if column_name not in valid_df.columns:
                raise ValueError(f"Cannot filter on '{column_name}'. Available columns: {valid_df.columns.tolist()}")
            valid_values = valid_df[column_name].dropna().unique()
            requested_values = attribute if isinstance(attribute, list) else [attribute]
    ...
    def selection_filters(self):
        ...
        active_filters = {
            column_name: value
            for column_name, value in named_filters.items()
    ...
    def application_time_label(self, treatment):
        return "PRE" if treatment == "PRE" else "POST"
    ...
    def build_agg_df(self):
        ...
        if  self.project_obj.project_type == "intrinsic_properties":
            ...
            if self.data_type == 'IF_IC': 
                ...
                filtered_df['time'] = filtered_df['treatment'].apply(self.application_time_label)
    ...
    def folder_file_AP_df(self, cell_id, folder_file,  V_array=None, I_array=None, command_array=None, sampling_rate=None):
        ...
        if file_data_type == 'APP_IC':
            ...
            drug_labels = [row_info['treatment'] if row_info['treatment'] == 'PRE' else 'POST' for _ in sweep_indices_all]
            current_injected = [
                protocol_value_at(loc, sweep)
                for loc, sweep in zip(peak_locs_corr_all, sweep_indices_all)
            ]
        ...
        for _, row in df.iterrows():
            ...
            if self.dynamic_search:
                ...
                result_rows.append({
                    ...
                    'delta': result['mean_diff'] if self.sweep_dependant_var() != 'sweep_inputR_MOhm' else result['percent_diff'],
                    'p_val': result['p_val'],
                    'latency_sweeps': 0,
...
class ApplicationResponse(Figure):
    ...
    def __post_init__(self):
        ...
        self.data = self.filter_n_minimum(self.raw_time_df)
        self.pre_post_df = self.build_pre_post_df(self.data, slice=self.slice)
...
class Histogram:
    ...
    def finalize_histogram(self, ax, fig, subgroup_name=None):
        ...
        ax.set_ylabel(unit_dict.get(self.dependant_var, self.dependant_var), fontsize=24)
        ax.set_xlabel("")
        ax.set_title(
    ...
    def plot_applications(self):
        ...
        for cell_id in cell_ids:
            ...
            for folder_file, cell_id, I_set, drug, drug_in, drug_out, RA_locs in cell_sub_df[['folder_file','cell_id', 'I_set', 'treatment', 'drug_in', 'drug_out', 'RA_locs']].values:
                ...
                AP_df = self.folder_file_AP_df(cell_id, folder_file, V_array, I_array, command_array, sampling_rate=sampling_rate_hz)
